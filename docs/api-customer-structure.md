...
管理画面から顧客のフリガナに `ヷヸヹヺ` を入力して保存したところ、API の応答では HTML 数値文字参照を
デコードしないリテラル文字列として返った。先頭のカタカナに続けて `&#12535;` `&#12536;` `&#12537;`
`&#12538;` が並ぶ形で、`html_entity_decode()` すると入力した文字に戻る。

同じ形の文字列を API 経由で `furigana` へ書き込もうとすると `422` / code `422250` ではなく
`422003` / `field: customer.furigana` /「フリガナを正しく入力してください。」で拒否された。
この値は API 経由では書き込めず、管理画面からのみ入る。

この観測は、[ADR 0017](adr/0017-opt-in-value-fallback.md) で応答側の値オブジェクトへ
フォールバックを導入する根拠になった。
...
| コード | メッセージ | 観測条件 |
| --- | --- | --- |
| 422003 | フリガナを正しく入力してください。 | `furigana` に数値文字参照を含む文字列を送ったとき |
