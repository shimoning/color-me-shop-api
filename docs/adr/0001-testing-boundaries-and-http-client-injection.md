...
- アクセストークンを受け取る `Client` の公開メソッドでは、トークン引数の `null` のみを「未指定」
  とし、空文字を含む非 `null` 値は明示指定として保持する。検証は基底 `Services\Service` に集約し、
  Service の生成時とリクエスト生成時の実効トークンが空文字なら `ParameterException` とする。
  空文字は厳密に `""` のみとし、`"0"` は有効な不透明文字列として受理する。これにより、明示した
  空文字を保持中の旧トークンへフォールバックさせず、別テナントの認証情報で通信することを防ぐ。
  出典: `1b68f8d`、`84406cc`、`714c883`、`def5b8e`。
...
トークンの `null` と空文字を区別する挙動は現行の `Client` で維持され、空文字の検証は
`Services\Service` に集約されている。直接生成時とリクエスト生成時の両方で空文字を拒否し、`"0"` は
有効なトークンとして使用する。PHPStan level 3 と抑制を使わない方針も、現行の `phpstan.neon`、
`composer.json` で維持されている。
