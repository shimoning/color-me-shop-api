...
| `PointState`: `assumed`, `fixed`, `canceled` | `Sales\Sale` の 3 ポイント状態、`Sales\SaleUpdateInput.pointState` | `sale` レスポンス: 同じ 3 値。`PUT /v1/sales/{sale_id}` のリクエストだけ `cenceled` | レスポンス差分なし。リクエストは `cenceled` 不足 / `canceled` 余り（仕様 typo の疑い、未検証） | 状態遷移。更新入力にも使用 | 無 |
