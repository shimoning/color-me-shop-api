 ...
 * これは公式 OpenAPI のグループ作成・更新 request の enum と一致する。応答で `members_only` を
 * 管理画面設定のグループでも観測 (2026-09-22): オーナーが管理画面で会員限定に設定した既存グループが
 * 一覧・単体とも `members_only` を返した。
 *
 ...
 * いない (2026-09-22 の 5 グループにも現れなかった) が、別のショップや設定の既存グループが応答で返す
 * 可能性を否定できないため、未知値として一覧全体が読めなくなる (0.11.0 以前の `Product\Group` で
 * 起きた) 失敗を避ける。書き込み側の
 ...
 * と「2026-09-22 の追加観測（管理画面で設定された既存グループの読み取り）」、docs/enum-openapi-audit.md。
 */
